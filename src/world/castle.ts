...
import { flameField, vapourField, type FireSource } from './effects';
import { detailedCandle, candelabrum, plate, goblet, bottle, turned, HALL_TABLES } from './props';
import { buildStairHall } from './staircases';
import { enrichRooms, type Interaction } from './enrichment';
import { random, type Materials, glowTexture } from './materials';

export type WorldObjects = { setHatSpeaking:(value:boolean)=>void; stairs: ReturnType<typeof buildStairHall>; interactions: Interaction[]; obstacles: Builder['obstacles']; candles: T.Points; water: T.Mesh; waterUniforms: {time:{value:number}}; flames: T.Points; portals: {position:T.Vector3; target:'astronomy'|'courtyard'}[]; animations: (t:number)=>void };
...
export function buildCastle(scene:T.Scene,m:Materials):WorldObjects {
  ...
  function candle(x:number,y:number,z:number,h=.55,floating=false) {
    detailedCandle(b,fire,x,y,z,h,!floating);
    if(floating)candlePts.push(x,y+h+.06,z);else glow(x,y+h+.4,z,.4);
  }
  function torch(x:number,y:number,z:number,ry=0) {
    b.box(x,y-.18,z,.2,.75,.25,m.iron,ry); b.cylinder(x,y+.18,z,.17,.3,m.gold,.28,8);
    fire.push({x,y:y+.32,z,size:.48});glow(x,y+.55,z,.55);
  }
  ...
  for(let i=0;i<76;i++) {
    const a=i/76*Math.PI*2,r=38+rand()*6,sy=12+rand()*11;const g=new T.IcosahedronGeometry(1,2);const p=g.getAttribute('position');
  ...
  b.box(-12.5,8.5,-22,1,17,44,m.sandstone,0,true);b.box(12.5,8.5,-22,1,17,44,m.sandstone,0,true);frontWall(0,-44,25,17,4.8);
  ...
  for(let z=-5;z>-44;z-=6) {
    for(const s of [-1,1]){
      ...
      b.opening(s*11.98,5,z,2.5,8,-s*Math.PI/2,false,true);
  ...
  for(const x of HALL_TABLES.xs) {
    ...
    for(let z=HALL_TABLES.firstPlaceZ;z>HALL_TABLES.lastPlaceZ;z-=2.3) {
      for(const dx of [-HALL_TABLES.plateOffset,HALL_TABLES.plateOffset]) {plate(b,x+dx,1.185,z);goblet(b,x+dx*.8,1.19,z+.47);b.box(x+dx+.31,1.205,z,.026,.012,.34,m.silver);b.box(x+dx-.31,1.205,z,.023,.012,.32,m.silver);}
    }
    for(let z=-12;z>-37;z-=6.3)candelabrum(b,fire,x,1.18,z);
  ...
  light(0,8,-15,125,32);light(0,8,-34,125,32);light(0,4,-1,65,14);
  ...
  light(-27,5,-9,115,24);light(-27,5,-30,110,24);
  ...
  for(const z of [-12,-24,-34])for(const x of [23,31]) {
    ...
    turned(b,x,1.14,z,[[0,0],[.3,0],[.51,.12],[.64,.3],[.65,.48],[.59,.68],[.58,.86],[.51,.86],[.52,.68],[.58,.48],[.57,.3],[.44,.17],[.26,.075],[0,.075]],m.iron,40);b.cylinder(x,1.93,z,.5,.025,m.green);for(let leg=0;leg<3;leg++){const a=leg/3*Math.PI*2;b.cylinder(x+Math.cos(a)*.38,1.16,z+Math.sin(a)*.38,.065,.18,m.iron,.05,12);}b.add(new T.TorusGeometry(.59,.08,8,24).rotateX(Math.PI/2),m.gold,x,2,z);
    for(const dx of [-1,1])b.add(new T.TorusGeometry(.16,.04,6,16),m.iron,x+dx*.69,1.6,z);
    for(let j=0;j<3;j++){const xx=x+1.1,zz=z-.9+j*.65;bottle(b,xx,1.135,zz,.8+j*.12);}
    candle(x-1.5,1.2,z-1,.6);
  }
  for(const z of [-9,-19,-29]){b.box(35.2,3,z,1.1,5,4.4,m.wood,0,true);for(let y=1;y<5;y++)for(let zz=z-1.7;zz<z+1.8;zz+=.6){bottle(b,34.5,y-.15,zz,.56+(Math.floor(zz*2)%3+1)*.1);}torch(18.7,3.5,z);}
  light(27,5,-9,85,24,0xc9c4a0);light(27,4,-27,95,26,0x83bead);
  ...
  for(const a of [0,.5,1,1.5]){const angle=a*Math.PI;const xx=30+Math.cos(angle)*6.8,zz=-47+Math.sin(angle)*6.8;b.tower(xx,zz,1.4,5,6,37);b.cylinder(xx,35.95,zz,.3,2.1,m.trim,1.45,20);b.beam(v(30+Math.cos(angle)*6.2,34.8,-47+Math.sin(angle)*6.2),v(xx,36,zz),.26,m.trim);}
  ...
  const terrain=new T.PlaneGeometry(1500,1500,100,100);terrain.rotateX(-Math.PI/2);const tp=terrain.getAttribute('position');
  ...
  const stairs=buildStairHall(scene,m,fire);b.obstacles.push(...stairs.obstacles);
  const enrichment=enrichRooms(scene,m,fire);b.obstacles.push(...enrichment.obstacles);
  b.finish();
  const fireFX=flameField(fire);scene.add(fireFX.mesh);
  const smoke= vapourField([23,31].flatMap(x=>[-12,-24,-34].map(z=>v(x,2,z))));scene.add(smoke.mesh);
  ...
  const pointMat=new T.PointsMaterial({size:.62,map:glowTexture(),transparent:true,blending:T.AdditiveBlending,depthWrite:false,vertexColors:true});const flames=new T.Points(pointGeo,pointMat);scene.add(flames);
  const cg=new T.BufferGeometry();cg.setAttribute('position',new T.Float32BufferAttribute(candlePts,3));const candles=new T.Points(cg,new T.PointsMaterial({size:.34,color:0xffc478,map:glowTexture(),transparent:true,blending:T.AdditiveBlending,depthWrite:false}));scene.add(candles);
  ...
  return {setHatSpeaking:enrichment.setHatSpeaking,obstacles:b.obstacles,stairs,interactions:enrichment.interactions,candles,flames,water,waterUniforms,portals,animations(t){
    portalRings.forEach((r,i)=>{r.rotation.z=t*(i%2?-.12:.12);});fireFX.update(t);smoke.update(t);enrichment.update(t);pointMat.opacity=.64+Math.sin(t*6)*.025;
  }};
}
