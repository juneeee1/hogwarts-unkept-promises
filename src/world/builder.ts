...
export function gothicOutline(w:number,h:number){
    const rise=w*.63,spring=h-rise;
...
export class Builder {
  ...
  obstacles: Obstacle[]=[];
  constructor(public scene:T.Object3D, public m:Materials) {}
  ...
  sphere(x:number,y:number,z:number,r:number,mat:T.Material,sx=1,sy=1,sz=1) {const g=new T.SphereGeometry(r,r<.035?8:r<.2?10:24,r<.035?6:r<.2?6:16);g.scale(sx,sy,sz);this.add(g,mat,x,y,z);}
  ...
  arch(x:number,y:number,z:number,w:number,h:number,depth=.32,ry=0,mat:T.Material=this.m.trim) {
    const thick=.22,points=gothicOutline(w,h);
    // One continuous moulding instead of dozens of overlapping rectangular segments.
    const outer:T.Vector2[]=[],inner:T.Vector2[]=[];
    for(let i=0;i<points.length;i++){
      const prev=points[Math.max(0,i-1)],next=points[Math.min(points.length-1,i+1)];const tangent=next.clone().sub(prev).normalize();const normal=new T.Vector2(-tangent.y,tangent.x).multiplyScalar(thick/2);
      outer.push(points[i].clone().add(normal));inner.push(points[i].clone().sub(normal));
    }
    const shape=new T.Shape([...outer,...inner.reverse()]);shape.closePath();const g=new T.ExtrudeGeometry(shape,{depth,bevelEnabled:false,steps:1});g.translate(0,0,-depth/2);const uv=g.getAttribute('uv');for(let i=0;i<uv.count;i++)uv.setXY(i,uv.getX(i)/4,uv.getY(i)/4);this.add(g,mat,x,y,z,ry);

  }
  opening(x:number,y:number,z:number,w:number,h:number,ry=0,lit=true,stained=false) {
    const s=new T.Shape(gothicOutline(w,h));s.closePath();
    const pane=new T.ShapeGeometry(s,16),uv=pane.getAttribute('uv');for(let i=0;i<uv.count;i++)uv.setXY(i,uv.getX(i)/w+.5,uv.getY(i)/h);this.add(pane,stained?(this.m.stainedGlass??this.m.blueWindow):lit?this.m.window:this.m.blueWindow,x,y,z,ry);
    ...
    const ring=new T.TorusGeometry(w*.18,.045,6,16);this.add(ring,this.m.darkStone,...local(0,h*.79,.09).toArray() as [number,number,number],ry);
  }
  roof(x:number,z:number,w:number,d:number,base:number,rise:number) {
    ...
    const g=new T.ExtrudeGeometry(shape,{depth:.45,bevelEnabled:false});this.add(g.clone(),this.m.stone,x,base,z+d/2-.2);this.add(g,this.m.stone,x,base,z-d/2-.2);
  }
  ...
  finish() {
    for(const [mat,geos]of this.buckets){const merged=mergeGeometries(geos,false);if(!merged)throw new Error('Geometry merge failed');const indexed=mergeVertices(merged,1e-4);merged.dispose();const mesh=new T.Mesh(indexed,mat);mesh.castShadow=mat!==this.m.flame&&mat!==this.m.window;mesh.receiveShadow=true;this.scene.add(mesh);geos.forEach(g=>g.dispose());}
    this.buckets.clear();
  }
}
