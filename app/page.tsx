...
export default function Home(){
  ...
  useEffect(()=>{let alive=true;let instance:CastleEngine|null=null;let boot:ReturnType<typeof setTimeout>;
    import('@/src/world/engine').then(({CastleEngine})=>{boot=setTimeout(()=>{if(!alive||!mount.current)return;try{instance=new CastleEngine(mount.current,s=>{if(!alive)return;setState(s);if(s.mode==='walk')setVisited(v=>v.includes(s.place)?v:[...v,s.place]);},setError);engine.current=instance;setReady(true);}catch(e){console.error(e);setError('暂时无法开启三维场景。请使用支持 WebGL 2 的浏览器重新打开。');}},80);}).catch(()=>setError('城堡资源载入失败，请检查网络后重试。'));
